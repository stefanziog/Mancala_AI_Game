...
class Kalaha:
    ...
    def get_score(self, player):
        ...
        if sum(self.board[7:13]) == 0 and self.board[6] > self.board[13] and sum(self.board[0:6]) > 0 and player == 0:
            return self.board[6] + sum(self.board[0:6])
        elif sum(self.board[0:6]) == 0 and self.board[13] > self.board[6] and sum(self.board[7:13]) > 0 and player != 0:
            return self.board[13] + sum(self.board[7:13])
        elif sum(self.board[0:6]) == 0 and player == 0:
            return self.board[6]
        elif sum(self.board[7:13]) == 0 and player != 0:
            return self.board[13]

        if player == 0 and sum(self.board[0:6]) != 0:
            return self.board[6]
        elif player != 0 and sum(self.board[7:13]) != 0:
            return self.board[13]
